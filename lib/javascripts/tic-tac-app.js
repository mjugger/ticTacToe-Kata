var t3Kata = angular.module('t3Kata',['ui.bootstrap']);
...
t3Kata.controller('t3Board',['$scope',function($scope){
	...
	$scope.humanMove = function(index){
			if($scope.t3Board[index] === undefined){
	 ...
	 */
	function cpuMove(){
	    var nextMove = minimax(2,$scope.cpu);
	    updateCell(nextMove[1],$scope.cpu);
	    console.log('evaluateBoard: ',evaluateBoard());
	}
